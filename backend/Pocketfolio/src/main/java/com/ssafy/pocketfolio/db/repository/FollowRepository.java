...
public interface FollowRepository extends JpaRepository<Follow, Long> {
	...
	@Query(value = "select ifnull((select follow_seq from follow where user_from = ?1 and user_to = user_seq), 0) as followSeq, " +
			"u.user_seq as userSeq, u.name, u.profile_pic as profilePic from user u inner join follow f on u.user_seq = f.user_from " +
	...
	@Query(value = "select ifnull((select follow_seq from follow where user_from = ?1 and user_to = user_seq), 0) as followSeq, " +
			"u.user_seq as userSeq, u.name, u.profile_pic as profilePic from user u inner join follow f on u.user_seq = f.user_to " +
