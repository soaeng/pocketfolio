...
public interface UserRepository extends JpaRepository<User, Long> {
	...
	@Query(value = "select u.user_seq, u.email, u.name, ifnull(u.profile_pic, \"\") as profilePic, u.birth, " +
			...
			"u.describe, u.blog_url from user as u where user_seq = ?1", nativeQuery = true)
	Optional<UserView> findProfileById(long userSeq);

	@Query(value = "select u.user_seq, u.email, u.name, ifnull(u.profile_pic, \"\") as profilePic, u.birth, " +
			"(select count(*) from follow where user_to = u.user_seq) as followerTotal, " +
			"(select count(*) from follow where user_from = u.user_seq) as followingTotal, " +
			"u.describe, u.blog_url from user as u", nativeQuery = true)
